...
@section('title', "Liste des Utilisateurs non enseignants")
@section('sidebar')
    @parent
				   ...
				   <form action="{{ route('user.save') }}" method="POST">	
					...
					@csrf
						...
						<div class="grid grid-cols-1 gap-6 mt-4">	
							<div>
								<label class="text-black dark:text-gray-200" for="nom">Nom</label>
							...
							<div>
								<label class="text-black dark:text-gray-200" for="prenoms">Prénoms</label>
								<input id="prenoms" name="prenoms" type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
							</div>     		
							
							<div>
								<label class="text-black dark:text-gray-200" for="email">Adresse e-mail</label>
...
theModalEdt.addEventListener('show.bs.modal', function (event) {
  ...
  var modalNom = theModalEdt.querySelector('#nom')
  var modalPrenoms = theModalEdt.querySelector('#prenoms')
  var modalEmail = theModalEdt.querySelector('#email')
  var modalId = theModalEdt.querySelector('#id')

  modalTitle.textContent = (theId == 0) ? "Nouvelle UE" : "Modifier l'UE";
  modalNom.value = (theId > 0) ? row_data.nom : '';
  modalPrenoms.value = (theId > 0) ? row_data.prenoms : '';
  modalEmail.value = (theId > 0) ? row_data.email : '';
...
const dataTable = new DataTable("#myTable", {	
    ...
    columns: [
		...
		{
			data: "email", title: "Adresse e-mail"
		},
		{
			data: "id",
            ...
            render: function(data, td, row, meta) {
				...
				let url_sup = "{{ route('user.delete', ['id' => ":id"]); }}";
