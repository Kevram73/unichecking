...
@section('title', "Type d'indisponibilités du corps professoral")
@section('sidebar')
    @parent
			  ...
			  <div class="modal-dialog">
				<div class="modal-content">
				  <div class="modal-header">
				   ...
				   <form action="{{ route('type_deplacement.save') }}" method="POST">	
					<div class="modal-body">			
					@csrf
						<input type="hidden" id="id" name="id">
						<div class="grid grid-cols-1 gap-6 mt-4">	
							<div>
								<label class="text-black dark:text-gray-200" for="designation">Type</label>
								<input id="designation" name="designation" type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
							</div>  
						</div>
					  </div>
					  <div class="modal-footer">
...
theModalDel.addEventListener('show.bs.modal', function (event) {
  ...
  modalTitle.textContent = "Supprimer le type de déplacement";
...
theModalEdt.addEventListener('show.bs.modal', function (event) {
  ...
  var modalType = theModalEdt.querySelector('#designation')
  var modalId = theModalEdt.querySelector('#id')

  modalTitle.textContent = (theId == 0) ? "Nouveau type" : "Modifier le type";
  modalType.value = (theId > 0) ? row_data.designation : '';
    ...
    columns: [
		...
		},
		{
            ...
            render: function(data, td, row, meta) {
				...
				let url_sup = "{{ route('type_deplacement.delete', ['id' => ":id"]); }}";
