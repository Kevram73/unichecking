			  ...
			  <div class="modal-dialog">
				<div class="modal-content">
				  <div class="modal-header">
				   ...
				   <form action="{{ route('universite.save') }}" method="POST">	
					<div class="modal-body">			
					@csrf
...
theModalDel.addEventListener('show.bs.modal', function (event) {
  ...
  modalTitle.textContent = "Supprimer le type de pièce d'identité";
...
theModalEdt.addEventListener('show.bs.modal', function (event) {
  ...
  var modalType = theModalEdt.querySelector('#nom')
  var modalId = theModalEdt.querySelector('#id')

  modalTitle.textContent = (theId == 0) ? "Nouvelle université" : "Modifier le nom de l'université";
  modalType.value = (theId > 0) ? row_data.nom : '';
    ...
    columns: [
		...
		},
		{
            ...
            render: function(data, td, row, meta) {
				...
				let url_sup = "{{ route('universite.delete', ['id' => ":id"]); }}";
