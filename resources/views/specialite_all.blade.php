...
@section('title', "Spécialités du corps professoral")
@section('sidebar')
    @parent
			  ...
			  <div class="modal-dialog">
				<div class="modal-content">
				  <div class="modal-header">
				   ...
				   <form action="{{ route('specialite.save') }}" method="POST">	
					<div class="modal-body">			
					@csrf
						<input type="hidden" id="id" name="id">
						<div class="grid grid-cols-1 gap-6 mt-4">	
							<div>
								<label class="text-black dark:text-gray-200" for="code">Code</label>
								<input id="code" name="code" type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
							</div>
							<div>
...
theModalEdt.addEventListener('show.bs.modal', function (event) {
  ...
  var modalCode = theModalEdt.querySelector('#code')
  var modalIntitule = theModalEdt.querySelector('#intitule')
  var modalId = theModalEdt.querySelector('#id')

  modalTitle.textContent = (theId == 0) ? "Nouvelle spécialité" : "Modifier la spécialité";
  modalCode.value = (theId > 0) ? row_data.code : '';
    ...
    columns: [
		...
		},
		{
            ...
            render: function(data, td, row, meta) {
				...
				let url_sup = "{{ route('specialite.delete', ['id' => ":id"]); }}";
				...
				"data-bs-url='" + url_sup + "' " +
				"data-bs-name='" + row.code + " - " + row.intitule + "' " +
