...
@section('title', "Listing des Unités d'Enseignements")
@section('sidebar')
    @parent
			  ...
			  <div class="modal-dialog">
				<div class="modal-content">
				  <div class="modal-header">
				   ...
				   <form action="{{ route('ue.save') }}" method="POST">	
					<div class="modal-body">			
					@csrf
...
theModalDel.addEventListener('show.bs.modal', function (event) {
  ...
  modalTitle.textContent = "Supprimer l'UE";
...
theModalEdt.addEventListener('show.bs.modal', function (event) {
  ...
  modalTitle.textContent = (theId == 0) ? "Nouvelle UE" : "Modifier l'UE";
    ...
    columns: [
		...
		},
		{
            ...
            render: function(data, td, row, meta) {
				let link_mod = "<a  style='color:blue' href='#' " +
				"data-bs-toggle='modal' " +
				"data-bs-target='#modalEdit' " +
				"data-bs-id='" + row.id + "' " +
				" ><i class='fas fa-edit'></i></a>";
				let url_sup = "{{ route('ue.delete', ['id' => ":id"]); }}";
